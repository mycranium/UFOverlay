function makeOverlays() {
    try {
        ...
        var success = false;
        while (!success) {
          var tmpData = importList();
          if (tmpData) {
            ovl.data = tmpData;
            success = true;
          } else {
            var retry = confirm("Hmmm... It looks like the file is not vali JSON data. Try again?", false, "Invalid JSON Data");
            if (!retry) {
              alert("No vallid file selected. Exiting script.");
              break;
            }
          }
        }
        ...
        function importList() { // Imports and parses JSON string and returns it as an array.
          try {
            ...
            alert("Oops. On line " + err.line + " of \"importList()\" you got " + err.message);
            return false;
          }
        }
        ...
        function buildOverlays() {
            ...
            ovl.compsFolder = makeCompsFolder(formatDate(true));
            generateComps(ovl.data, ovl.compsFolder, ovl.savePath);
