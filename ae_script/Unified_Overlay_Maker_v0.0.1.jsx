function makeOverlays() {
  try {
    ...
    function makeCompsFolder(compType, compsFolderDate) { // Makes a new folder for the generated comps, named using the date and time. Returns a project folder object.
      var compsFolder = null; // why???
      if (!compsFolder) { // Creates a new compsFolder if it doesn't already exist.
        compsFolder = getItemByTypeAndName("Folder", "__" + compType + "_" + compsFolderDate) || app.project.items.addFolder("__" + compType + "_" + compsFolderDate);
      }
      return compsFolder;
    }
    ...
    function buildTargetCompName(entry) { // Constructs the name of the comp to duplicate for the overlay being created
      ...
      var side = entry.side;
      var compId = myData.type + "-" + style + side;
...
function generateComps(compsFolder, filePath) { // Generates the required new comps and places them in the new comps folder.
  try {
    ...
    function buildOverlays() { // Function that finally creates all the overlays
      ...
      try { var myCompsFolder = makeCompsFolder(myType, formatDate(true)); } catch (err) { alertErrors("buildOverlays", err, true); }
      try { generateComps(myCompsFolder, overlayGlobals.overlaySavePath); } catch (err) { alertErrors("buildOverlays", err, true); }
